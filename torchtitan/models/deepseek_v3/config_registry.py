...
def deepseek_v3_16b_nvfp4() -> Trainer.Config:
    ...
    config.model_spec = model_registry(
        ...
        converters=[
            # TEST-ONLY: kernel_preference pinned to triton to prove the value
            # reaches the kernels rather than only the logger. Revert before
            # merging -- the shipped default is "auto".
            NVFP4LinearConverter.Config(
                model_compile_enabled=model_compile_enabled,
